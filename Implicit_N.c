...
/*created by Arya H*/
#define tMax 1
#define M_PI 3.1415269 // visual studio does not automatically know M_PI
#define dx (M_PI/10)
#define xf  M_PI
#define xi 0
#define D 1.0
...
void heat_implicit_N() {
	...
	for (i = 0; i <= M; i++) {
		// u[i][0] = 0; we do not know what u0 is. therefore we leave it blank
		u[i][N] = 0;
	}
	for (t = 1; t <= M; t++) {
		alpha[0] = a;
...
double u0(double x) { return cos((n - 0.5)*x); }

double exact(double x, double t) { return exp(-1 * pow(n - 0.5, 2)*t)*cos((n - 0.5)*x); }
